...
export async function onRequestPost(context: { request: Request; env: Env }) {
  try {
    const adminToken = safeText(context.env.LICENSE_ADMIN_TOKEN, 256);
    if (!adminToken) {
      return json({ ok: false, error: 'missing-env', key: 'LICENSE_ADMIN_TOKEN' }, { status: 500 });
    }
    ...
    // 仅 id：只能查是否已兑换（未兑换时无法判断是否过期/积分是多少，除非你还保留 code）
    if (idFromBody) {
      const row = await db
        .prepare(
          'SELECT id, points, amount_cents as amountCents, issued_at as issuedAt, expires_at as expiresAt, redeemed_at as redeemedAt, redeemed_account_id as redeemedAccountId FROM license_redemptions WHERE id = ? LIMIT 1'
      ...
      if (!row) {
        return json({ ok: true, status: 'not-redeemed-or-unknown', id: idFromBody, message: '未查到核销记录：可能未兑换，或请用 code 查询以判断未用/过期' });
      }
